...
const columnHelper = createColumnHelper();
  ...
  <div className="animate-pulse bg-gray-200 h-20 w-full rounded-md"></div>
);
...
const Category = () => {
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor('actions', { header: 'Actions', cell: ({ row }) => (<DropdownMenu modal={false}><DropdownMenuTrigger asChild><Button variant='ghost' className='data-[state=open]:bg-muted flex h-8 w-8 p-0'><Ellipsis className='h-4 w-4'/></Button></DropdownMenuTrigger><DropdownMenuContent align='end' className='w-[160px]'><DropdownMenuItem onClick={() => { dispatch(setSelectedCategory(row.original)); setEdit(true); setOpen(true); }}>Edit<DropdownMenuShortcut><PenLine size={16}/></DropdownMenuShortcut></DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={() => { dispatch(setSelectedCategory(row.original)); setDeleteModal(true); }} className='text-red-500!'>Delete<DropdownMenuShortcut><Trash size={16}/></DropdownMenuShortcut></DropdownMenuItem></DropdownMenuContent></DropdownMenu>) })
  ], [dispatch]);
  ...
  const smCardsData = [
    { name: 'Total Categories', totalPrice: totalRows ?? 0, iconColor: 'bg-blue-100', textColor: 'text-blue-600', btnIcon: <Archive size={16}/> },
  ...
  return (
    <>
      ...
      <div className='py-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4'>
        {listLoading
          ? Array.from({ length: smCardsData.length }).map((_, i) => <SmCardSkeleton key={i} />)
          : smCardsData.map((card, i) => <SmCard key={i} {...card} />)}
      </div>

      <div className='flex gap-3 items-center mb-3'>
        <Input placeholder='Search category by name' className='h-8 w-[150px] sm:w-[350px]' value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
              ...
              {table.getHeaderGroups().map((headerGroup, i) => (
                <TableRow key={i}>
                  {headerGroup.headers.map((header, index) => (
                    <TableHead key={index}>{flexRender(header.column.columnDef.header, header.getContext())}</TableHead>
                  ))}
                </TableRow>
              ))}
            </TableHeader>
