...
// Thunks
export const fetchAllItem = asyncThunkRequest('item/fetchAll', (params) => getAllItem(params));
export const createItem = asyncThunkRequest('item/create', (body) => addItem(body));
export const editItem = asyncThunkRequest('item/update', ({ id, body }) => updateItem(id, body));
...
const normalizeItem = (item) => ({
  ...
  category: item.category || (item.categoryId ? { id: item.categoryId, name: item.categoryName || 'Unknown' } : null),
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAllItem.fulfilled, (state, { payload }) => {
        ...
        state.data = items.map(normalizeItem);
      ...
      .addCase(createItem.fulfilled, (state, { payload }) => {
        const newItem = normalizeItem(payload?.itemData || payload);
      ...
      .addCase(editItem.fulfilled, (state, { payload }) => {
        const updated = normalizeItem(payload?.itemData || payload?.data || payload);
        const index = state.data.findIndex(item => item.itemId === updated.itemId);
        if (index !== -1) {
      ...
      .addCase(removeItem.fulfilled, (state, { payload }) => {
        const removedId = payload?.itemId || payload?.id || payload?.data?.itemId;
      ...
      .addCase(fetchItemOptions.fulfilled, (state, { payload }) => {
        ...
        state.data = (payload?.itemData || []).map(normalizeItem);
      })
